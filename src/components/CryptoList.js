...
const CryptoList = ({ cryptoList, userHoldings, setUserHoldings, setTotalWorth }) => {
    ...
    const calculateTotalWorth = () => {
        ...
        for(let i = 0; i < getUpdatedData.length; i++) {
            let amount = getUpdatedData[i].quote.INR.price * userHoldings[getUpdatedData[i].id].coinQuantity;
            totalAmount += amount;
        }
